...
static void renderOpenGL(GLFWwindow* window, int shaderProgramID)
{
    ...
    glm::mat4 projection = glm::ortho(-51.0f, 51.0f, -51.0f, 51.0f, -1.f, 1.f);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, bufferIDs[2]);
    glVertexAttribPointer(0, 2, GL_FLOAT, false, 2 * sizeof(float), 0);
    glUniform2fv(translationLocation, 1, glm::value_ptr(agent.Position()));
    glDrawArrays(GL_LINE_LOOP, 0, 36);

    glUniform4fv(colourLocation, 1, red);
    glVertexAttribPointer(0, 2, GL_FLOAT, false, 2 * sizeof(float), 0);
    glUniform2fv(translationLocation, 1, glm::value_ptr(agent.Position() + agent.Velocity()));
    glDrawArrays(GL_LINE_LOOP, 0, 36);
}
